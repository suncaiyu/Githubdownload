...
QVariant TreeItem::data(int column) const
{
    if (column < mItemData.size()) {
        ...
        nameList << "name" << "url" << "download_url" << "type" << "size";
        if (column > nameList.size()) {
            return QVariant();
        }
        return mItemData[nameList[column]];
    }
    //if (mLevel == 1)
    //{
    //    //一级节点，班级
...
void TreeItem::setData(int column, QVariant value)
{
    ...
    QStringList nameList;
    nameList << "name" << "url" << "download_url" << "type" << "size";
    mItemData[nameList[column]] = value.toString();
}
